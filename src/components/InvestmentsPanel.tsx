  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Total Invested */}
        <div className="bg-theme-card p-5 rounded border border-theme-card-border flex items-center gap-4">
          <div className="p-3.5 bg-theme-bg text-[#d4af37] rounded border border-theme-card-border">
            <Briefcase className="h-6 w-6" />
          </div>
          <div>
            <span className="text-[10px] text-theme-muted block font-bold uppercase tracking-wider font-mono">Total Custodiado</span>
            <span className="text-xl font-bold font-mono text-theme-title">{formatBRL(totalInvested)}</span>
          </div>
        </div>
        ...
        <div className="bg-theme-card p-5 rounded border border-theme-card-border flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] text-theme-muted block font-bold uppercase tracking-wider font-mono">Rentabilidade Ponderada</span>
            <span className="text-xl font-bold font-mono text-theme-title">{weightedYield.toFixed(2)}% <span className="text-xs font-normal text-theme-muted font-mono">a.a.</span></span>
          </div>
        </div>
      ...
      <div className="bg-theme-card rounded border border-theme-card-border overflow-hidden">
        <div className="p-5 border-b border-theme-card-border flex items-center justify-between bg-theme-input">
          <h4 className="font-semibold text-theme-title text-sm uppercase tracking-wider font-mono">Minha Carteira de Ativos</h4>
          <span className="text-xs font-mono text-theme-muted uppercase tracking-widest">{investments.length} Ativos Ativos</span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-theme-card-border text-sm">
              {investments.length > 0 ? (
                investments.map((inv) => (
                  <tr key={inv.id} className="hover:bg-theme-hover transition">
                    <td className="py-4 px-6 font-bold text-theme-title font-mono">{inv.name}</td>
                    <td className="py-4 px-6">
                      <span className="bg-theme-bg text-theme-text border border-theme-card-border text-xs font-mono px-2 py-0.5 rounded">
                        {inv.type}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-theme-muted font-mono text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-theme-input border border-theme-card-border focus:border-[#d4af37] rounded py-2.5 px-3.5 outline-none text-theme-text font-medium text-sm transition cursor-pointer font-mono"
                  >
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="12.5"
                      className="w-full bg-theme-input border border-theme-card-border focus:border-[#d4af37] rounded py-2.5 pl-3.5 pr-8 outline-none text-theme-text font-mono font-medium text-sm transition"
                    />
                    <Percent className="absolute right-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-theme-muted" />
                  </div>
                </div>
